...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Logic tamamen kaldırıldı, direkt ana sayfaya
    router.push("/");
  };
      ...
      <div className="max-w-md w-full space-y-8 card-surface border border-white/10 rounded-2xl p-8 shadow-xl">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="rounded-md shadow-sm space-y-4">
            <div>
              ...
              <input
                ...
                type="email"
                autoComplete="email"
                defaultValue="kaşarmusti@gmail.com"
                className="block w-full px-3 py-2 border border-white/10 bg-slate-900/70 placeholder-slate-400 text-white rounded-md focus:outline-none focus:ring-cyan-400 focus:border-cyan-400 sm:text-sm opacit-50 cursor-not-allowed"
                placeholder={t.email}
              />
            </div>
            <div>
          ...
          <div>
            <button
              type="submit"
              className="w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-slate-900 bg-cyan-300 hover:bg-cyan-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-400"
            >
              {t.submit}
            </button>
          </div>
